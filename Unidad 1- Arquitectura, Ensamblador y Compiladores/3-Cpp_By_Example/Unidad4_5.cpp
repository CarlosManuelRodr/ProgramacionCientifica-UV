...
void DibujaRecuadro()
{
    ...
    for (unsigned i = 0; i < colores.GetNumRows(); i++)
    {
        for (unsigned j = 0; j < colores.GetNumCols(); j++)
        {
            // Llena la matriz colores del color rojo.
            colores[i][j] = Color((char) 255, (char) 0, (char) 0);
        }
    }
    Plot(colores, "rojo.bmp");
}
...
void DibujaMandelbrot()
{
    ...
    Plot(mandelbrot, "mandelbrot.bmp");
}
...
void DibujaMandelbrotColor()
{
    ...
    Plot(mandelbrot, "mandelbrot_color.bmp");
}
